...
#      ├─ 【6-23】 _on_schedule_toggle()   예약 버튼 4가지 분기 처리
#      ├─ 【6-24】 _ask_scheduled_start()  기록 시작 예약 시각 입력 창
#      ├─ 【6-25】 _register_schedule_timer()   시작 예약 폴링 타이머
#      ├─ 【6-26】 _ask_scheduled_stop()   기록 중지 예약 시각 입력 창
#      ├─ 【6-27】 _register_stop_schedule_timer() 중지 예약 폴링 타이머
#      ├─ 【6-28】 _stop_monitoring_silent()  예약 자동 중지 (확인창 없음)
#      ├─ 【6-29】 _open_save_file()          기록 파일 바로 열기
#      ├─ 【6-30】 _open_save_folder()        기록 파일 폴더 열기
#      ├─ 【6-31】 _show_api_status()      API 호출 현황 대화상자
#      ├─ 【6-32】 _show_program_info()    프로그램 정보 대화상자
#      ├─ 【6-33】 _on_toggle()            기록 시작/중지 토글 (예약·이전기록 팝업 포함)
#      ├─ 【6-34】 _start_monitoring()     모니터링 시작 (중복 실행 가드 포함)
#      ├─ 【6-35】 _clear_recorded_data()  기록창·recorded_data 초기화
#      ├─ 【6-36】 _stop_monitoring()      모니터링 중지 (중지예약 팝업 분기 포함)
#      ├─ 【6-37】 _main_loop()            백그라운드 갱신 루프 스레드
#      ├─ 【6-38】 _refresh_data()         한 번의 데이터 갱신 실행
#      ├─ 【6-39】 _process_routes()  ★    버스 위치 분석·운행 판정 핵심
#      ├─ 【6-40】 _record()               운행 이벤트 기록
#      ├─ 【6-41】 _perform_auto_save()    자동 엑셀 저장 (락 보호)
#      ├─ 【6-42】 _core_excel_save()      엑셀 파일 저장 (날짜별 시트)
#      ├─ 【6-43】 _axs()                  엑셀 시트 스타일 적용
#      ├─ 【6-44】 _ask_interval()         갱신 주기 대화상자 (유효 정의)
#      ├─ 【6-45】 _show_api_status()      API 현황 대화상자 (유효 정의)
#      ├─ 【6-46】 _show_program_info()    프로그램 정보 대화상자 (유효 정의)
...
# ──────────────────────────────────────────────────────────
    def _show_key_input(self):
        ...
        def _on_verified(rm_str, rb_str, key_m, key_b):
            # _do_ok 바깥(_show_key_input 스코프)에 정의 →
            # dlg.exec() 반환 후에도 disconnect 가 정상 참조 가능
...
# ──────────────────────────────────────────────────────────
# 【6-27】 _register_stop_schedule_timer()
...
# ──────────────────────────────────────────────────────────
# 【6-31】 _show_api_status()  ← 첫 번째 정의 (줄 2078의 두 번째 정의로 덮어씌워짐)
...
# ──────────────────────────────────────────────────────────
# 【6-32】 _show_program_info()  ← 첫 번째 정의 (줄 2116의 두 번째 정의로 덮어씌워짐)
...
# ──────────────────────────────────────────────────────────
# 【6-39】 _process_routes()  ★ 운행 기록 판정 핵심 로직 ★
#
...
# ──────────────────────────────────────────────────────────
# 【6-40】 _record(idx, ft, rnm, vn, sn, sa, status)
...
# ──────────────────────────────────────────────────────────
# 【6-42】 _core_excel_save(tp, sc=False)
...
# ──────────────────────────────────────────────────────────
# 【6-45】 _show_api_status()  ← 두 번째(유효) 정의
...
# ──────────────────────────────────────────────────────────
# 【6-46】 _show_program_info()  ← 두 번째(유효) 정의
