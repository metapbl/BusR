...
#      └─ 【1-3】 외부 선택 라이브러리 (requests, pandas, openpyxl)
# 【2】 전역 상수
#      ├─ 【2-1】 프로그램 버전 상수 (APP_VERSION)
#      ├─ 【2-2】 API 인증키 (API_KEY_MAIN, API_KEY_BACK - 대진여객 전용, 코드에 고정)
#      ├─ 【2-3】 글꼴 이름 (FONT_FAMILY, FONT_MONO)
#      ├─ 【2-4】 아이콘/이미지 데이터 (ICON_B64, GG_IMG_B64 - Base64)
#      ├─ 【2-5】 정류소 정보 테이블 (STATION_INFO - ID→ARS번호·이름)
#      ├─ 【2-6】 노선 설정 테이블 (ROUTE_SETUP - 노선별 고정 정류소 ID)
#      ├─ 【2-7】 API URL 상수 (URL_POS1 ~ URL_SRCH)
#      ├─ 【2-8】 노선 종류 이름표·색상표 (ROUTE_TYPE_LABEL, ROUTE_TYPE_COLOR)
#      ├─ 【2-9】 노선 메뉴 순서·기본 지도 노선 (ROUTE_MENU_ORDER, DEFAULT_MAP_ROUTE)
#      └─ 【2-10】 지도 그리기 크기/위치 상수 (CELL_W, CELL_H, PAD_X 등)
# 【3】 유틸리티 함수 및 클래스
...
#      ├─ 【6-12】 log()                   로그 시그널 발행
#      ├─ 【6-13】 _init_routes()    ★     노선 정보 자동 초기화 (백그라운드)
#      ├─ 【6-14】 _init_route_map_panels() 노선별 지도 패널 생성 [슬롯]
#      ├─ 【6-15】 _route_map_select()     특정 노선 지도를 화면 앞으로 전환
#      ├─ 【6-16】 fetch_api()       ★     서울시 버스 API 공통 호출
#      ├─ 【6-17】 _fetch_first_time()     첫차 시각 조회
#      ├─ 【6-18】 _on_toggle()            기록 시작/중지 토글 (예약·이전기록 팝업 포함)
#      ├─ 【6-19】 _start_monitoring()     모니터링 시작 (중복 실행 가드 포함)
#      ├─ 【6-20】 _clear_recorded_data()  기록창·recorded_data 초기화
#      ├─ 【6-21】 _stop_monitoring()      모니터링 중지 (중지예약 팝업 분기 포함)
#      ├─ 【6-22】 _main_loop()            백그라운드 갱신 루프 스레드
#      ├─ 【6-23】 _refresh_data()         한 번의 데이터 갱신 실행
#      ├─ 【6-24】 _process_routes()  ★    버스 위치 분석·운행 판정 핵심
#      ├─ 【6-25】 _record()               운행 이벤트 기록
#      ├─ 【6-26】 _perform_auto_save()    자동 엑셀 저장 (락 보호)
#      ├─ 【6-27】 _core_excel_save()      엑셀 파일 저장 (날짜별 시트)
#      ├─ 【6-28】 _write_source_sheet()   엑셀 "출처" 시트 작성 (공공데이터 4종 출처표시)
#      ├─ 【6-29】 _axs()                  엑셀 시트 스타일 적용
#      ├─ 【6-30】 _ask_interval()         갱신 주기 입력 대화상자
#      ├─ 【6-31】 _on_schedule_toggle()   예약 버튼 4가지 분기 처리
#      ├─ 【6-32】 _ask_scheduled_start()  기록 시작 예약 시각 입력 창
#      ├─ 【6-33】 _register_schedule_timer()   시작 예약 폴링 타이머
#      ├─ 【6-34】 _ask_scheduled_stop()   기록 중지 예약 시각 입력 창
#      ├─ 【6-35】 _register_stop_schedule_timer() 중지 예약 폴링 타이머
#      ├─ 【6-36】 _stop_monitoring_silent()  예약 자동 중지 (확인창 없음)
#      ├─ 【6-37】 _show_api_status()      API 호출 현황 대화상자
#      ├─ 【6-38】 _show_program_info()    프로그램 정보 대화상자
#      └─ 【6-39】 closeEvent()            창 닫기 이벤트 처리
# 【7】 프로그램 진입점 (if __name__ == "__main__")
# ══════════════════════════════════════════════════════════
# ============================================================
# DJ_Bus_Drive_Recorder_v1.52
# ============================================================
...
# base64     : 이진 데이터↔ASCII 인코딩. 이미지를 코드에 내장할 때 사용.
...
# ══════════════════════════════════════════════════════════
# 【2-2】 API 인증키 상수
#   서울시 공공데이터 API 호출에 필요한 인증키.
#   Seoul_Bus_Drive_Recorder와 달리 여기서는 코드에 직접 고정되어 있어
#   프로그램을 다시 컴파일하지 않는 한 변경할 수 없다.
#
#   API_KEY_MAIN: 메인 인증키 (64자리 16진수 문자열)
#   API_KEY_BACK: 보조 인증키 (메인 한도 초과 시 자동 전환)
#   ※ 인증키 교체 시 이 두 줄의 값만 바꾸면 됨.
#   ※ 이 두 키는 대진여객으로부터 위탁받아 본 프로그램 전용으로 발급된
#     인증키이며, 소스 코드가 공개 저장소에 올라가는 경우 그대로 노출된다.
#     인증키 유출 시의 트래픽 한도·이용약관 준수 책임은 발급 명의자(대진여객)
#     에게 있으므로, 이 파일을 재배포하기 전에 반드시 발급 명의자와
#     공개 여부를 협의하고, 필요 시 키를 교체해야 한다.
# ══════════════════════════════════════════════════════════
API_KEY_MAIN = "0b811fe84980051212b99cbdb27c8372f39aed7d029a4b94eac0463ae0510a36"
API_KEY_BACK = "ab358f4401965b9c41e51cb691696d68b5704fb8f974a5ff0c61c5f5ee450785"
...
# 【6-1】 __init__()
#   메인 창 초기화. 처리 순서:
#   1) 창 제목("대진여객 버스 운행기록 수집 프로그램 v1.52")·크기·아이콘 설정
...
# ──────────────────────────────────────────────────────────
        super().__init__()
        self.setWindowTitle("대진여객 버스 운행기록 수집 프로그램 v1.52")
        ...
        self.api_stats_today = {"POS1": 0, "POS2": 0, "SLST": 0, "RINF": 0, "기타": 0}
        self.api_stats_yesterday = {"POS1": 0, "POS2": 0, "SLST": 0, "RINF": 0, "기타": 0}
...
# ──────────────────────────────────────────────────────────
# 【6-13】 _init_routes()  ★ SeoulBusRecorder에는 없는 DJ 전용 함수 ★
...
#
#   ※ 이 함수가 완료되기 전에는 [기록 시작] 버튼이 비활성화됨.
...
# ──────────────────────────────────────────────────────────
# 【6-14】 _init_route_map_panels() [슬롯]
...
# ──────────────────────────────────────────────────────────
# 【6-16】 fetch_api(url, params) → XML root | tuple | None  ★ API 공통 호출 ★
#   API_KEY_MAIN → API_KEY_BACK 순으로 API 호출 시도.
#
#   반환값:
...
# ──────────────────────────────────────────────────────────
        lh = 0
        kc = sum(1 for k in [API_KEY_MAIN, API_KEY_BACK] if k)
        for key in [API_KEY_MAIN, API_KEY_BACK]:
            if not key:
                continue
            try:
                p = dict(params)
...
# ──────────────────────────────────────────────────────────
# 【6-17】 _fetch_first_time(rid) → "HH:MM" or None
...
# ──────────────────────────────────────────────────────────
# 【6-24】 _process_routes()  ★ 운행 기록 판정 핵심 로직 ★
#
...
# ──────────────────────────────────────────────────────────
# 【6-25】 _record(idx, ft, rnm, vn, sn, sa, status)
#   운행 이벤트(출발/도착)를 기록.
...
# ──────────────────────────────────────────────────────────
# 【6-27】 _core_excel_save(tp, sc=False)
...
# ──────────────────────────────────────────────────────────
    def _write_source_sheet(self, wb):
        ...
        rows = [
            ...
            ("소스 코드", "https://github.com/metapbl/BusR"),
            ("", ""),
            ("■ 유의사항", ""),
...
# ──────────────────────────────────────────────────────────
# 【6-35】 _register_stop_schedule_timer()
...
# ──────────────────────────────────────────────────────────
        ...
        notice = ("<p><b>■ 이용 안내</b></p>"
                  ...
                  "<li>이 프로그램의 인증키는 대진여객 6개 고정 노선 모니터링 전용으로 소스 코드에 "
                  "고정되어 있으며, 이용자가 별도로 입력하지 않습니다. 인증키는 대진여객으로부터 "
                  "위탁받아 발급된 것으로, 발급 명의자의 사전 동의 없이 이 파일을 재배포하거나 "
                  "공개 저장소에 그대로 올리지 마십시오.</li>"
                  "<li>인증키 관리 및 API 이용약관 준수 책임은 인증키 발급 명의자에게 있습니다.</li>"
                  "</ul>")
